...
<div class="mx-auto max-w-3xl px-4 py-12 text-zinc-300">
	...
	<p class="mb-8 text-lg text-zinc-400">
		...
		<strong class="font-medium text-zinc-200">block merges</strong> when GO/NO-GO blockers remain.
		Not Lighthouse. Same launch judgment you already trust.
	</p>
	...
	<section class="mb-10 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-6">
		...
		<ul class="list-disc space-y-2 pl-6 text-sm text-zinc-400">
			...
			<li>Score below <code class="rounded bg-zinc-800 px-1.5 py-0.5 text-sky-300">PREFLIGHT_MIN_SCORE</code> (default 80)</li>
			<li>Any P0 check failure — exposed secrets, missing privacy page, HTTPS, noindex, robots blocking Google, etc.</li>
		</ul>
	</section>
	...
	<section class="mb-10">
		...
		<p class="mb-4 text-sm text-zinc-500">
			Add repository secret <code class="rounded bg-zinc-800 px-1.5 py-0.5 text-sky-300">PREFLIGHT_GATE_URL</code>
			with your production URL (e.g. <code class="rounded bg-zinc-800 px-1.5 py-0.5">https://your-app.com</code>).
		</p>
		<pre
			class="overflow-x-auto rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-xs leading-relaxed text-zinc-300"
		><code>{hostedGateYaml}</code></pre>
		<ul class="mt-4 list-disc space-y-2 pl-6 text-sm text-zinc-400">
			...
			<li>
				<strong class="text-zinc-300">Job summary</strong> — the same markdown lands in the Actions
				run summary automatically.
			</li>
			<li>
	...
	<section class="mb-10">
		...
		<pre
			class="overflow-x-auto rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-xs leading-relaxed text-zinc-300"
		><code>{hostedScript}</code></pre>
		<p class="mt-3 text-sm text-zinc-500">
			Exit code <strong class="text-zinc-400">0</strong> = pass · <strong class="text-zinc-400">1</strong> = fail ·
	...
	<section class="mb-10 rounded-2xl border border-sky-900/40 bg-sky-950/20 p-6">
		...
		<ol class="list-decimal space-y-2 pl-6 text-sm text-zinc-400">
			<li>Scan free at <a class="text-sky-400 hover:underline" href="/">preflight.latham.cloud</a></li>
