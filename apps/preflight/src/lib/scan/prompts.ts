...
export function fixPrompt(id: string, context: { url: string; message?: string }): string {
	...
	const templates: Record<string, string> = {
		...
		stripe: `${base}Verify Stripe before launch: create Checkout Sessions on the server, keep live secret keys server-only in environment/secret storage, verify webhooks with the Stripe-Signature header and endpoint secret, fulfill access only from paid/completed events with idempotent handlers, handle failed/canceled states, and provide billing self-service for customers.`,
		'checkout-server-owned': `${base}Move checkout creation behind a server endpoint. The browser may request a checkout, but the server must create Stripe Checkout Sessions or PaymentIntents with trusted price IDs, authenticated customer identity, and server-only secret keys. Return only the session URL or session id to the client; never let client code choose secret checkout parameters or hold sk_live keys.`,
		'webhook-event-coverage': `${base}Expand the Stripe webhook handler to cover the full subscription lifecycle: checkout.session.completed, checkout.session.async_payment_succeeded, checkout.session.async_payment_failed, invoice.payment_failed, and customer.subscription.deleted. Use idempotent fulfillment keyed by Stripe event/session/subscription ids so retries do not double-grant access, and update failed/canceled states to revoke or pause access.`,
		'entitlement-fulfillment': `${base}Wire payment events to product access. On paid/completed checkout or subscription activation, grant the user/team entitlement in your database; on failed payment, canceled checkout, refunded payment, or customer.subscription.deleted, revoke or downgrade access. Make the handler idempotent and store the Stripe customer/subscription/session mapping for recovery.`,
		'payment-env-safety': `${base}Remove hardcoded payment secrets from source, tests, fixtures, and public bundles. Rotate any exposed sk_live or provider secret, then load live payment secrets only from server environment bindings or secret storage. Keep publishable keys client-side only, validate that live/test keys are separated, and block secret values from logs and generated artifacts.`,
		...
		'workflow-immutable-action-pins': `${base}Pin every external GitHub Action to a full 40-character commit SHA after verifying the SHA belongs to the expected action repository. Keep Dependabot or Renovate rules to update those pins deliberately instead of accepting moving tags silently.`,
		'codeql-code-scanning': `${base}Enable GitHub CodeQL code scanning. Use GitHub default setup when available, or add a .github/workflows/codeql.yml workflow using github/codeql-action/init and github/codeql-action/analyze on pull requests, main, and a scheduled run.`,
		'deploy-job-dependencies': `${base}Make every deploy, release, publish, or production job wait for the quality/security jobs in the same GitHub Actions workflow. Add a job-level needs value such as needs: [verify, security] or needs: verify. Do not rely on job order or step inputs named needs; GitHub Actions jobs run in parallel by default unless jobs.<job_id>.needs is set.`,
		'deploy-job-environment': `${base}Add a job-level GitHub Actions environment to each deploy, release, publish, or production job, for example environment: production or environment: { name: production, url: ... }. Configure the environment with required reviewers, branch rules, wait timers, or environment-scoped secrets before relying on it as a deploy gate.`,
