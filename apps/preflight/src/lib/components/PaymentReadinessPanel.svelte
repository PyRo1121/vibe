...
<section class="mb-10 rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
	...
	{#if paymentReadiness.blockers.length > 0}
		<div class="mt-5">
			<p class="text-xs font-semibold tracking-wide text-red-300 uppercase">Access blockers</p>
	...
	{#if paymentReadiness.checked.length > 0}
		<p class="mt-5 text-xs text-zinc-500">
			Checked {paymentReadiness.checked.length} customer access signal{paymentReadiness.checked
				.length === 1
				? ''
				: 's'}.
		</p>
